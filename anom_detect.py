...
class anom_detect():
	...
	def plot(self,data_label=None,left=None,right=None,bottom=None,top=None,from_console=False):
		...
		ax1.legend()
		if from_console is True:
			plt.show()
		elif from_console is False:
			pass
		else:
			raise ValueError('Invalid input for from_console parameter')
	...
	def evaluate(self,data,anom_detect=True):
		...
		if anom_detect:
			...
			anoma_points = pd.DataFrame(pd.DataFrame(df[df.columns[0]]).iloc[outliers_index,0].sort_index()) #df[['sunspots']]
